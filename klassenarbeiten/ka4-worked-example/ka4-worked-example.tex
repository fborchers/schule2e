...
%% Seitenlayout (exam-Dokumentenklasse) ----
\extrawidth{1.59cm}%
% Kopfzeile:
...
% oder alternativ Seitenlayout mit 'geometry':
% \usepackage{geometry}
% \geometry{%
% 	margin=1.5cm
% }
...
\begin{klassenarbeitskopf}{Klassenarbeit 1}{Mathematik}{10c}{10.10.2024}
	...
	\small
	\partialgradetable{ohneHM}[h][questions] 
	\partialgradetable{mitHM}[h][questions] \\
...
\begin{parts}
	% Ankreuzaufgaben ---
	\part Kreuze jeweils an, ob die Aussage wahr (W) oder falsch (F) ist:
\begin{truefalses}
\truefalse Ein Vektor $\vec{u}$ heißt \emph{parallel} zu einem anderen Vektor $\vec{v}$, wenn für $t\neq0$ gilt: $\vec{u} = t\cdot \vec{v}$.
\truefalse Wenn bei einem Vektor zwei Komponenten negativ sind, dann müssen bei einem anderen parallelen Vektor diese Komponenten auch negativ sein. 
\truefalse Wenn bei einem Vektor eine Komponente null ist, dann ist diese Komponente auch bei allen anderen parallelen Vektoren gleich null.
\end{truefalses}

% Widerlege die Aussage (als AFB3)
\part Widerlege die folgende Aussage:
\begin{itemize}
	\item Wenn zwei Geraden einen gemeinsamen Spurpunkt haben, dann sind sie parallel oder identisch.
\end{itemize}
...
\part Berechne die Geschwindigkeit des Flugzeugs~1  und gib sie in \unit{\km\per\hour} an.

% Der folgende Text erscheint eine Ebene höher:
\uplevel{Der Luftraum, der von den Flugzeugen genutzt wird, wird von zwei verschiedenen Radarstationen überwacht. Die "`Übergabe"' der Flugzeuge erfolgt, 
wenn die Flugzeuge die $x_2x_3$-Ebene durchfliegen.}
% Alternativ kann man auch die Umgebung verwenden "EnvUplevel",
% dann können auch Abbildungen usw. enthalten sein. 

% Es folgt die weitere Aufgabenstellung in entspr. Einrückung:
